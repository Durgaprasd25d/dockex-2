...
import { FiCreditCard, FiUploadCloud, FiSearch, FiX, FiAlertTriangle, FiFile } from "react-icons/fi";
...
function UploadPAN() {
    ...
    const upload = async () => {
        if (!file) {
            setErrorMsg("Please select or drop a PAN Card image first");
            return;
        }
            ...
            <div className="card-header-clean">
                <div>
                    <h2 className="card-title-clean">
                        <FiCreditCard className="text-purple" style={{ fontSize: '20px', color: '#a78bfa' }} />
                        <span>PAN Card OCR</span>
                    </h2>
                    <p className="card-subtitle mb-0">Upload or drop Income Tax PAN card image</p>
                </div>
                <span className="badge-tag badge-blue" style={{ background: 'rgba(139, 92, 246, 0.15)', color: '#a78bfa', borderColor: 'rgba(139, 92, 246, 0.3)' }}>Type: PAN</span>
            </div>
            ...
            <div className="p-4">
                ...
                {!preview ? (
                    ...
                    >
                        ...
                        />
                        <div className="upload-icon-wrapper" style={{ background: 'rgba(139, 92, 246, 0.1)', color: 'var(--purple)' }}>
                            <FiUploadCloud />
                        </div>
                        <h4 className="fw-semibold text-white mb-1" style={{ fontSize: '15px' }}>
                            Click to browse or drag & drop PAN card
                        </h4>
                    ...
                    <div className="mb-3">
                        <div className="preview-box mb-3">
                            <img src={preview} alt="PAN Preview" />
                            <button
                                type="button"
                ...
                >
                    {loading ? (
                        <>
                            ...
                            <span>Extracting PAN Data...</span>
                        </>
                    ) : (
                        <>
                            <FiSearch style={{ fontSize: '16px' }} />
                            <span>Extract PAN Details</span>
                        </>
                    )}
                </button>

                <ResultCard data={result} text={rawText} docType="PAN Card" />
            </div>
        </div>
    );
}
